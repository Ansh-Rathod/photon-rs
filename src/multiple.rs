...
fn build_horizontal_gradient(width: usize, height: usize, start_x: i32, end_x: i32) -> Vec<f32> {
...
fn build_vertical_gradient(width: usize, height: usize, start_y: i32, end_y: i32) -> Vec<f32> {
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            if min_x <= foot_x && foot_x <= max_x && min_y <= foot_y && foot_y <= max_y {
                ...
                let fill_bottom_right = start_x < end_x && start_y < end_y && foot_x > max_x;
                let fill_bottom_left = start_x > end_x && start_y < end_y && foot_x < min_x;
                let fill_top_right = start_x < end_x && start_y > end_y && foot_y < min_y;
                let fill_top_left = start_x > end_x && start_y > end_y && foot_y < min_y;
                if fill_bottom_right || fill_bottom_left || fill_top_right || fill_top_left {
                    gradient[pos] = 1.0;
                }
            }
        }
...
pub fn fade(
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            let res_r = ((img1_r * opacity_img1) + (img2_r * opacity_img2)).clamp(0.0, 255.0) as u8;
            let res_g = ((img1_g * opacity_img1) + (img2_g * opacity_img2)).clamp(0.0, 255.0) as u8;
            let res_b = ((img1_b * opacity_img1) + (img2_b * opacity_img2)).clamp(0.0, 255.0) as u8;
